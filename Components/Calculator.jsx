...
export default function Calculator(){
    ...
    return (
        ...
        <input type="number" className="--input" placeholder="num1" value={numbers.num1} name="num1" onChange={handleChange}/>
        <input type="number" className="--input" placeholder="num1" value={numbers.num2} name="num2" onChange={handleChange}/>
        </div>
        <div className="--buttons-div">
            <button className="--button" onClick={() => calculate("add")}>{add}</button>
            <button className="--button" onClick={() => calculate("subtract")}>{subtract}</button>
            <button className="--button" onClick={() => calculate("multiply")}>{multiply}</button>
            <button className="--button" onClick={() => calculate("divide")}>{div}</button>
        </div>
        <div className="--errors">
            {error && <p style={{color: error==="Error!" ? 'red' : 'green'}}>{error}</p>}
